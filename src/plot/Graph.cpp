...
void Graph::setGraphBox(const sf::RectangleShape& graphBox) {
    ...
    _graphTexture.create(_graphBox.getLocalBounds().width,
                         _graphBox.getLocalBounds().height);
}
...
void Graph::setPlotBox(const sf::RectangleShape& plotBox) {
    ...
    _plotTexture.create(plotBox.getLocalBounds().width,
                        plotBox.getLocalBounds().height, settings);
...
void Graph::setTitle(const std::string& title) { _title = title; }

void Graph::setFontSize(const int& fontSize) { _fontSize = fontSize; }

void Graph::setFont(const sf::Font& font) { _font = font; }

void Graph::setBottomLeft(const sf::Vector2f& bottomLeft) {
    _bottomLeft = bottomLeft;
}

void Graph::setTopRight(const sf::Vector2f& topRight) { _topRight = topRight; }
...
void Graph::setText() {
    ...
    _text.setPosition(_graphBox.getGlobalBounds().left +
                          0.5f * _graphBox.getGlobalBounds().width,
                      0.5f * (_plotBox.getGlobalBounds().top +
                              _graphBox.getGlobalBounds().top +
                              _plotBox.getGlobalBounds().height +
                              _graphBox.getGlobalBounds().height));
...
void Graph::setXScale(const double& x0, const double& xf, const int& N) {
    ...
    while (number <= xf) {
        ...
        out << std::fixed;
        out.precision(floor(log10(idealScale)) < 0 ? -floor(log10(idealScale))
                                                   : 0);
        out << number;
        s = out.str();
        aux.setString(s);
        ...
        aux.setPosition(sf::Vector2f(
            ...
            _plotBox.getGlobalBounds().top + _plotBox.getGlobalBounds().height +
                0.1 * (_graphBox.getGlobalBounds().top -
                       _plotBox.getGlobalBounds().top +
                       _graphBox.getGlobalBounds().height -
                       _plotBox.getGlobalBounds().height)));
...
void Graph::setYScale(const double& y0, const double& yf, const int& N) {
    ...
    while (number <= yf) {
        ...
        aux.setPosition(
            sf::Vector2f(_plotBox.getGlobalBounds().left -
                             0.1 * (_plotBox.getGlobalBounds().left -
...
sf::RectangleShape Graph::getGraphBox() const { return _graphBox; }

sf::RectangleShape Graph::getPlotBox() const { return _plotBox; }

std::string Graph::getTitle() const { return _title; }

int Graph::getFontSize() const { return _fontSize; }

sf::Font Graph::getFont() const { return _font; }

sf::Vector2f Graph::getBottomLeft() const { return _bottomLeft; }

sf::Vector2f Graph::getTopRight() const { return _topRight; }

sf::Texture Graph::getPlotTexture() const { return _plotTexture.getTexture(); }

sf::Text Graph::getText() const { return _text; }

// Constructor Implementation

Graph::Graph(const sf::RectangleShape& graphBox,
             const sf::RectangleShape& plotBox, const sf::Vector2f& bottomLeft,
             const sf::Vector2f& topRight, const std::string& title,
             const int& fontSize, const sf::Font& font) {
...
sf::Vector2f Graph::convertPoint(const sf::Vector2f& point) const {
    ...
    realPoint.x = 0.5f + (point.x - _bottomLeft.x) *
                             ((_plotTexture.getTexture().getSize().x - 1) /
                              (_topRight.x - _bottomLeft.x));
    realPoint.y = 0.5f + (_topRight.y - point.y) *
                             ((_plotTexture.getTexture().getSize().y - 1) /
                              (_topRight.y - _bottomLeft.y));
    return realPoint;
}

// Basic Plot Functions Implementations
void Graph::plotClear(const sf::Color& color) { _plotTexture.clear(color); }
...
void Graph::plotData(const std::vector<sf::Vector2f>& data,
                     const bool& drawLines, const bool& drawPoints,
                     const sf::Color& lineColor, const sf::Color& pointColor) {
...
void Graph::plotFunction(const std::function<double(double)>& f,
...
void Graph::terminate() { _terminate = true; }

bool Graph::codigo_do_luca(std::complex<double> const& c, double x0, double y0,
                           double xf, double yf, bool coloca) {
    auto x = real(c);
    auto y = imag(c);
    ...
    for (int di = -1; di <= 1; ++di) {
        for (int dj = -1; dj <= 1; ++dj) {
            int i2 = i + di, j2 = j + dj;
            if (i2 < 0 || i2 >= 6 * _Nx || j2 < 0 || j2 >= 6 * _Ny) continue;
    ...
    if (_collision[i][j]) return true;
    if (coloca) _collision[i][j] = true;
    return false;
}

void Graph::plotRelation(double (*f)(double, double),
    ...
    while (!_terminate) {
        ...
        for (int it = 0; it < 10000 && !_terminate; it++) {
            ...
            sf::Vector2<double> p =
                p0 -
                sf::Vector2<double>(derivativex, derivativey) * f(p0.x, p0.y) /
                    (derivativey * derivativey + derivativex * derivativex);
        ...
        while (true) {
            auto dir =
                (grad / std::abs(grad)) * std::complex<double>(0, -1) * step;
            auto p1 = p;
            p += dir;
...

        {
            ...
            auto derivativex = fdx(std::real(paux), std::imag(paux));
            auto derivativey = fdy(std::real(paux), std::imag(paux));
...
void Graph::display() {
    ...
    sprite.setPosition(_plotBox.getGlobalBounds().left +
                           0.5f * _plotBox.getGlobalBounds().width -
                           _graphBox.getGlobalBounds().left,
                       _plotBox.getGlobalBounds().top +
                           0.5f * _plotBox.getGlobalBounds().height -
                           _graphBox.getGlobalBounds().top);

    _graphTexture.clear(sf::Color::Black);

    sf::RectangleShape aux = _plotBox;
    aux.move(-_graphBox.getGlobalBounds().left,
             -_graphBox.getGlobalBounds().top);
    ...
    for (sf::Text v : _xScale) {
        sf::Text aux_ = v;
        aux_.move(-_graphBox.getGlobalBounds().left,
                  -_graphBox.getGlobalBounds().top);
    ...
    for (; ini < x; ini++) {
        ...
        float d =
            std::sqrt((_lines1[ini].position.x - _lines2[ini].position.x) *
                      ...
                      (_lines1[ini].position.y - _lines2[ini].position.y) *
                          (_lines1[ini].position.y - _lines2[ini].position.y));
        ...
        line.setPosition(
            0.5 * (_lines1[ini].position.x + _lines2[ini].position.x),
            0.5 * (_lines1[ini].position.y + _lines2[ini].position.y));
        line.setFillColor(lineColor);
        double theta;
        double pi = 3.1415926535898;

        theta = (180.0 / pi) *
                std::atan2((_lines1[ini].position.y - _lines2[ini].position.y),
                           (_lines1[ini].position.x - _lines2[ini].position.x));
        line.rotate(theta);
